...
class App:
    ...
    def create_tree(self, node_id):
        ...
        # elif node_type == ' "uml:OpaqueExpression"':
        #    node_name = 'Constratint'
            ...
            for key, value in obj.items():
                ...
                if match:
                    ...
                    if new_key == "Comment":
                        ...
                        new_value.update({"body": body})  # add 'body' to the 'attributes' dictionary
                        item = {"name": new_key, "uuid": uuid, "attributes": new_value}
                    elif self.is_int(new_key):
                        item = {"value": int(new_key), "uuid": uuid, "type:": "integer"}
                        new_key = "parameter"
                    elif self.is_float(new_key):
                        item = {"value": float(new_key), "uuid": uuid, "type:": "float"}
                        new_key = "parameter"
                    elif self.is_boolean(new_key):
                        item = {"value": new_key.lower() == 'true', "uuid": uuid, "type": "boolean"}
                        new_key = "parameter"
                    else:
                        item = {"name": new_key, "uuid": uuid, "attributes": new_value}
                else:
